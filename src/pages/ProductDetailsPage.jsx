...
const getRepeatingProducts = (allProducts, currentProductId, repeatCount = 5) => {
    ...
    const filteredProducts = Object.values(allProducts).filter(p => p.id !== currentProductId);
...
function ProductDetailsPage() {
  ...
  const mockProducts = {
    '1': {
      ...
      // Todas as miniaturas agora são a mesma imagem
      thumbnailImages: [
        producImage5Preview,
        producImage5Preview,
        producImage5Preview,
    ...
    '2': {
      ...
      rating: 4.2,
      reviews: 75,
      currentPrice: "$100", // Formato de preço ajustado
      oldPrice: "$200", // Formato de preço ajustado
    ...
    '3': { 
      ...
      currentPrice: "$100", oldPrice: "$200", discount: "30%", // Ajustado formato de preço e desconto
    ...
    '4': { 
      ...
      currentPrice: "$70", oldPrice: null, discount: null, // Ajustado formato de preço e desconto
  ...
  const scrollCarousel = (direction) => {
    if (carouselRef.current) {
      ...
      // Define a largura da margem (mr-6 é 24px)
      const cardMarginRight = 24; // Valor em pixels para mr-6 no Tailwind
      const cardTotalWidth = cardWidth + cardMarginRight; // Largura do card + sua margem
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="lg:flex lg:gap-8 items-stretch">
          ...
          <div className="lg:w-1/2 relative flex flex-col items-center mb-6 lg:mb-0">
            ...
            <div className="w-full bg-purple-100 rounded-lg p-4 mb-4 relative"> {/* Adicionado 'relative' */}
                ...
                {product.thumbnailImages && product.thumbnailImages.length > 1 && ( // Mostra botões apenas se houver mais de 1 imagem
                    <>
                        <button
                            className="absolute left-2 top-1/2 transform -translate-y-1/2 p-2 z-10" // Removido shadow-md, hover:bg-opacity-100, transition-colors
                        ...
                        <button
                            className="absolute right-2 top-1/2 transform -translate-y-1/2 p-2 z-10" // Removido shadow-md, hover:bg-opacity-100, transition-colors
            ...
            <div className="flex justify-between mt-4 w-full"> 
              {product.thumbnailImages && product.thumbnailImages.map((thumb, index) => (
                <div
                  ...
                  style={{ backgroundColor: THUMBNAIL_COLORS[index % THUMBNAIL_COLORS.length] }} // Aplica a cor de fundo pelo índice
                  onClick={() => handleThumbnailClick(thumb, index)}
                >
                  <img
                    src={producImage5Preview} // A imagem do sapato é a mesma para todas as miniaturas
          ...
          <div className="lg:w-1/2 pt-4 lg:pt-0 flex flex-col min-h-full flex-grow">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2 flex-wrap">
                {product.colors && product.colors.map((color, index) => (
                  <button
                    key={index}
                    className={`w-8 h-8 rounded-full border-2 ${color === '#FF0000' ? 'border-pink-600' : 'border-gray-300 hover:border-pink-600'} focus:outline-none focus:ring-2 focus:ring-pink-600 focus:ring-opacity-50`}
            ...
            {/* Botão Comprar */}
            <button className="w-full bg-orange-500 text-white font-bold py-3 px-6 rounded-lg hover:bg-orange-600 transition-colors duration-200 text-lg shadow-md mt-auto">
              COMPRAR
            </button>
          </div>
        </div>
